...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_status') {
    ...
    // Check current status to prevent unnecessary updates
    $stmt = $pdo->prepare("
        SELECT status 
        FROM reports 
        WHERE reportID = :id
...
// 4) On GET: fetch assigned reports (with optional filter)
$itID = $_SESSION['user_id'];
$sql = "
  SELECT 
    r.reportID,
      ...
      <table class="data-table">
        ...
        <tbody>
          <?php if (empty($reports)): ?>
            <tr><td colspan="7" style="text-align:center;">No assigned reports.</td></tr>
      ...
      select.addEventListener('change', async function() {
        ...
        try {
          ...
          if (json.success) {
            ...
            const badge = row.querySelector('td:nth-child(6) .status');
