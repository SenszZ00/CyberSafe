...
<head>
  ...
  <style>
    /* paste your existing styles from admDashSUBMITTEDPREP.html here */
    body {
        display: flex;
    ...
    /* Filter form (matching Report Log design) */
    .filter-form {
      display: flex;
      gap: 10px;
      align-items: center;
...
<body>
        ...
        <tbody>
          ...
          <?php else: foreach ($reports as $r):
              ...
              switch ($r['status']) {
                ...
                case 'under review': $cls='status-investigating';  $lbl='Under Investigation';break;
            ...
            <tr data-report="<?= $r['reportID'] ?>">
              ...
              <td>
                <?php if ($r['attachments'] !== null): ?>
                  <button class="action-btn attachment-btn"
                          onclick="viewAttachments(<?= $r['reportID'] ?>)">
              ...
              <td>
                <select class="assign-dropdown"
                        onchange="assignReport(<?= $r['reportID'] ?>, this.value)">
                  <option value="">— Unassigned —</option>
                  <?php foreach ($itPersonnel as $it): ?>
                    <option value="<?= $it['userID'] ?>"
                      <?= $it['userID'] === $r['itPersonnelID'] ? 'selected' : '' ?>>
  ...
  <script>
    // Enable & adapt the input and Go button based on filter selection
    function updateInput() {
      ...
      if (sel === 'submissionDate') {
        ...
        inp.type = 'text';
        inp.placeholder = sel === 'incidentType'
          ? 'e.g. malware'
          : 'e.g. pending';
      }
    }
    ...
    // existing menu & assignment scripts unchanged
    function toggleMenu() {
